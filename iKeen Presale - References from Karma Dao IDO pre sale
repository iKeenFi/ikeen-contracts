...
contract iKeenPreSale is Ownable {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  address public alphaISKEEN;
  address public DAOAddress;
  address public mim; // ADD USDC USDC.E ...
  ...
  mapping(address => bool) public boughtISKEEN;
  ...
  function purchaseaISKEEN(uint256 _val, address _user)
    external
    payable
    returns (bool)
  {
    ...
    require(boughtISKEEN[_user] == false, "Already participated");
    require(startTimestamp < block.timestamp, "Not started yet");

    boughtISKEEN[_user] = true;
    ...
    IERC20(alphaISKEEN).safeTransfer(msg.sender, _purchaseAmount);
    return true;
  }
